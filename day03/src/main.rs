...
fn solve(program: &[Rucksack], parts: u8) -> (u32, u32) {
...
fn main() {
    ...
    let code = read_input(format!("{}{}", root, "/input/day03.txt").as_str());
...
mod day03 {
    ...
    fn part1() {
        use crate::*;
        let root = env::current_dir().unwrap().display().to_string();
        let input = read_input(format!("{}{}", root, "/../input/sample03.txt").as_str());
